...
public class CompanyAdapter extends BaseQuickAdapter<WxdepartmentListBean, BaseViewHolder> {
    ...
    public CompanyAdapter(int layoutResId, @Nullable List<WxdepartmentListBean> data) {
        super(layoutResId, data);
    }

    public CompanyAdapter(@Nullable List<WxdepartmentListBean> data) {
        super(data);
    }

    public CompanyAdapter(int layoutResId) {
        super(layoutResId);
    }

    @Override
    protected void convert(BaseViewHolder helper, final WxdepartmentListBean item) {
        final TextView txCompanyTitle = helper.getView(R.id.tx_company_title);
        helper.setText(R.id.tx_company_title, item.getWxdepartName());
        ...
        txCompanyTitle.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (txC.getVisibility() == View.VISIBLE || txV.getVisibility() == View.VISIBLE) {
                    rvC.setVisibility(View.GONE);
                    txC.setVisibility(View.GONE);
                    rvV.setVisibility(View.GONE);
                    txV.setVisibility(View.GONE);
                } else {
                    txC.setVisibility(item.getComprehensive().getDeviceList() == null || item.getComprehensive().getDeviceList().size() == 0 ? View.GONE : View.VISIBLE);
                    rvC.setVisibility(View.GONE);
                    txV.setVisibility(item.getVisualization().getLineList() == null || item.getVisualization().getLineList().size() == 0 ? View.GONE : View.VISIBLE);
                    rvV.setVisibility(View.GONE);
                }
            }
        });
        ...
        if (item.getVisualization().getLineList() == null || item.getVisualization().getLineList().size() == 0) {
            ...
            vAdapter.setEnableLoadMore(false);
