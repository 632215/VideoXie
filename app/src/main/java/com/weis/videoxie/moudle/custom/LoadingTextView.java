...
public class LoadingTextView extends FrameLayout {
    ...
    public LoadingTextView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mLoadingView = (LoadingView) view.findViewById(R.id.loading_tx_view);
        txLoading = (TextView) view.findViewById(R.id.tx_loading);

        setTextStyle(style);
        mParentLayout.setGravity(gravity);

        txLoading.setTextColor(textColor);
        txLoading.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
        setText(text);
    }
    ...
    public void setTextStyle(int style) {
        if (style == -1) {
            ...
            if (style != mParentLayout.getOrientation()) {
                ...
                LinearLayout.LayoutParams layoutParams = (LinearLayout.LayoutParams) txLoading.getLayoutParams();
    ...
    public void setText(CharSequence text) {
        txLoading.setText(text);
    }

    public void setText(int resid) {
        txLoading.setText(resid);
    }
}
