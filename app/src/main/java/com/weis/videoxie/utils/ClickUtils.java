...
public final class ClickUtils {
    ...
    public static boolean isFastDoubleClick(int tagId, long diff) {
        ...
        if (lastTagId == tagId && lastClickTime > 0 && dTime < diff) {
            LogUtils.w(TAG, "isFastDoubleClick 无效点击 => tagId: " + tagId + ", diff: " + diff);
            return true;
        }
        LogUtils.w(TAG, "isFastDoubleClick 有效点击 => tagId: " + tagId + ", diff: " + diff);
        lastTagId = tagId;
        lastClickTime = cTime;
        return false;
    }
    ...
    public static boolean isFastDoubleClick(String tag, long diff){
        ...
        if (lastTime > 0 && dTime < diff) {
            LogUtils.w(TAG, "isFastDoubleClick 无效点击 => tag: " + tag + ", diff: " + diff);
            return true;
        }
        LogUtils.w(TAG, "isFastDoubleClick 有效点击 => tag: " + tag + ", diff: " + diff);
        // 保存上次点击时间
    ...
    public static boolean isFastDoubleClick(Object obj, long diff){
        ...
        // 判断时间是否超过
        if (lastTime > 0 && dTime < diff) {
            LogUtils.w(TAG, "isFastDoubleClick 无效点击 => obj: " + obj + ", diff: " + diff);
            return true;
        }
        LogUtils.w(TAG, "isFastDoubleClick 有效点击 => obj: " + obj + ", diff: " + diff);
        // 保存上次点击时间
        mapRecords.put(tag, cTime);
        return false;
    }
